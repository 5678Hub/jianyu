#!/usr/bin/env python3
"""v82-fix52: r0003 food 简化（去除全路径前缀）

用户反馈：L2 级中的食品中文名称「饮料类：果蔬汁类及其饮料：含浆果及小粒水果的果蔬汁类及其饮料(葡萄汁除外)」太繁琐
修改：直接引用 PDF 中的名称「含浆果及小粒水果的果蔬汁类及其饮料(葡萄汁除外)」
...
for it in pb["items"]:
    if it.get("id") == "r0003":
        ...
        it["food"] = "含浆果及小粒水果的果蔬汁类及其饮料(葡萄汁除外)"
        print(f"[UPDATE] r0003 food:")
        print(f"  old: {old_food!r}")
        print(f"  new: {it['food']!r}")
        break
...
# bump 版本号 3 处
OLD_META = "v82-fix51-delete-r0005-keep-concentrate-only-2026-09-01"
NEW_META = "v82-fix52-simplify-r0003-food-2026-09-01"
OLD_TITLE = "[v82-fix51]"
NEW_TITLE = "[v82-fix52]"
