#!/usr/bin/env python3
"""
生成核对 Excel v2 — 4 个 sheet:
  Sheet 1: 按食品类别查询(按污染物排序)
  Sheet 2: 食品大类统计
  Sheet 3: 脚注说明
...
OUT_PATH = "data/gb2762/gb2762_2025_食品类别_v2.xlsx"
...
def main():
    ...
    # 排序
    def sort_key(it):
        return (L1_RANK.get(it["L1"], 999), it["L1"], it["L2"], it["L3"], it["污染物"])
    ...
    wb.save(OUT_PATH)
    print(f"OK: {OUT_PATH}")
    print(f"  Sheet 1: {ws1.max_row} rows")
    print(f"  Sheet 2: {ws2.max_row} rows")
    print(f"  Sheet 3: {ws3.max_row} rows")
    print(f"  Sheet 4: {ws4.max_row} rows")
